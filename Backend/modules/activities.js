...
router.get('/recent_activity/:userId/:groupId/:sortFlag', checkAuth, async (req, res) => {
    ...
    kafka.make_request('recentActivity', req.params, function (err, results) {
        console.log('in recentActivity results');

        if (err) {
            console.log("Inside err");
            res.status(500).send("Error while getting recent activity")
...
router.post('/settleup', checkAuth, async (req, res) => {
    ...
    kafka.make_request('settleUp', req.body, function (err, results) {
        console.log('in settleUp results');

        if (err) {
            console.log("Inside err");
            res.status(500).send("Error while settling up")
        } else if( err == null && results == null){
            console.log("Inside err");
            res.status(500).send("Error while settling up")
        }
...
router.get('/settleup/dropdown/:userId', checkAuth, async (req, res) => {
    ...
    kafka.make_request('settleUpDropDown', req.params, function (err, results) {
        console.log('in settleUpDropDown results');

        if (err) {
            console.log("Inside err");
            res.status(500).send("Error while getting settle up dropdown")
        } else if( err == null && results == null){
            console.log("Inside err");
            res.status(500).send("Error while getting settle up dropdown")
        }
