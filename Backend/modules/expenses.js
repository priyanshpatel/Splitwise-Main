...
router.post('/add', async (req, res) => {
    ...
    try {
        ...
        let groupSchemaDoc = await groupSchema.findOne({ _id: req.body.groupId }, 'acceptedUsers groupBalances')
        ...
        groupMembers.forEach(member => {
            ...
            groupMembers.forEach(member => {
                if (member == req.body.userId) {
                    ...
                    let groupBalancesDoc = await groupDoc.save()
                    console.log("Group Balances Successfully updated", groupBalancesDoc)

                    debtSchemaDoc = await debtSchema.findOne({
                        $and: [
                            {
                                $or: [
